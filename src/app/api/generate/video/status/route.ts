...
// ─── GET /api/generate/video/status?content_id=X ────────────────────────────
...
export async function GET(request: Request) {
  try {
    ...
    if (!log) {
      return Response.json(
        { success: false, error: 'No video generation task found for this content' } satisfies ApiResponse,
        { status: 404 }
      );
    }
    ...
    if (log.output_result) {
      try {
        ...
        currentSection = result.current_section ?? null;
        totalSections = result.total_sections ?? null;
      } catch {
        // ignore parse errors
      }
    }
